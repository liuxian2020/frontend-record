...
// /"idle", 
  ...
  // loader.load(`${this.publicPath}model/glbxz.com6031.FBX`, object => {//加载路径fbx文件
      ...
      // loader.load("./assets/5264_Summoner_Show3.fbx", (gltf) => {
        // console.log(gltf。, '[===')
        // this.scene.add(gltf)
        ...
        for (var i = 0; i < gltf.animations.length; i++) {
          this.actions[i] = this.mixer.clipAction(gltf.animations[i]);
          // console.log(this.actions[i].getClip().name, '====动画名称' )
        }
        ...
        // const clip = THREE.AnimationClip.findByName( gltf.animations, 'Yes' );
        ...
        let time = setInterval(() => {
          ...
          this.actionIndex = Math.floor(Math.random() * this.actionList.length);
          this.actions[this.actionIndex].play()
          this.actions[this.actionIndex].loop = THREE.LoopOnce;

        }, 60000)

        // this.actionIndex = Math.floor(Math.random() * this.actionList.length);
        // this.actions[this.actionIndex].play()
        // this.actions.loop = THREE.LoopOnce;
      //   console.log(this.actionList[name], this.allAnimations)
